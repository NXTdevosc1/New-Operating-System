    ...
    UINT64 Flags, UINT64 NumPages, void **Ptr)
{
    ...
    if (NosInitData->TotalPagesCount - NosInitData->AllocatedPagesCount < NumPages)
            ...
            if (PhysicalMem->Groups[c].Present)
            {
                ...
                while (_BitScanForward64(&Index, Mask))
                {
                    ...
                    if (Mem->Attributes & MM_DESCRIPTOR_ALLOCATED)
                        continue;
                    if (_interlockedbittestandset(&Mem->Attributes, MM_DESCRIPTOR_BUSY))
                        continue;

                    if (((UINT64)Mem->PhysicalAddress +
                         ExcessBytes(Mem->PhysicalAddress, Align) + NumBytes) < 0xFFFFFFFF &&
                        Mem->NumPages >= ((ExcessBytes(Mem->PhysicalAddress, Align) + NumBytes) >> 12))
                    {
        ...
        for (int c = 0; c < 0x40; c++)
        {
            if (PhysicalMem->Groups[c].Present)
            {
                ...
                while (_BitScanForward64(&Index, Mask))
                {
                    ...
                    NOS_MEMORY_DESCRIPTOR *Mem = &PhysicalMem->Groups[c].MemoryDescriptors[Index];
                    if (Mem->Attributes & MM_DESCRIPTOR_ALLOCATED ||
                        (UINT64)Mem->PhysicalAddress < 0x100000000)
                        continue;
                    if (_interlockedbittestandset(&Mem->Attributes, MM_DESCRIPTOR_BUSY))
                        continue;
